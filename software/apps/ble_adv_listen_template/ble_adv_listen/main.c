...
void ble_evt_adv_report(ble_evt_t const* p_ble_evt) {
  ...
  if (ble_addr[5] == 0xC0 && ble_addr[4] == 0x98 && ble_addr[3] == 0xE5 && ble_addr[2] == 0x42) {

    // print advertisement data
    // first, find index of manufacturer data
    uint8_t i = 0;
    while(1) {
        uint8_t len = adv_buf[i++];
        ...
        if (i >= adv_len) {
            printf("Did not find manufacturer specific data\n");
            break;
        }
    }

    // skip over company id
    i+=2;
